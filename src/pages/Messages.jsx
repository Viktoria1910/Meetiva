...
export default function Messages() {
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center bg-white rounded-2xl p-10" style={{ border: '1px solid #DDE3DE' }}>
            ...
            <button onClick={() => navigate('/login')}
              ...
              style={{ background: '#A7A5D0' }}>Prijava</button>
          </div>
        </div>
      </div>
    );
  ...
  const acceptOffer = (msg) => {
    ...
    try { localStorage.setItem('meetiva_reservations', JSON.stringify(updated)); } catch {}
    setConvs(prev => prev.map(c => c.id === activeId
      ? { ...c, messages: c.messages.map(m => m.id === msg.id ? { ...m, offerAccepted: true } : m) }
      : c
    ));
  };
  ...
  const declineOffer = (msgId) => {
    setConvs(prev => prev.map(c => c.id === activeId
      ? { ...c, messages: c.messages.map(m => m.id === msgId ? { ...m, offerDeclined: true } : m) }
      : c
    ));
  };
            ...
            {convs.map(c => (
              <button key={c.id} onClick={() => setActive(c.id)}
                ...
                <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-lg"
                  style={{ background: '#E8F0EA' }}>{c.avatar}</div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-semibold text-xs truncate" style={{ color: '#2B3132' }}>{c.provider}</p>
                    <span className="text-xs flex-shrink-0" style={{ color: '#8A9192' }}>{c.time}</span>
                  </div>
                  <p className="text-xs truncate mt-0.5" style={{ color: '#8A9192' }}>{c.lastMessage}</p>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 flex flex-col rounded-2xl overflow-hidden"
            ...
            <div className="flex items-center gap-3 px-5 py-3" style={{ background: '#505A5B' }}>
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-lg"
                style={{ background: '#E8F0EA' }}>{activeConv.avatar}</div>
              <div>
                <p className="font-bold text-sm text-white">{activeConv.provider}</p>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.6)' }}>{activeConv.category}</p>
              </div>
            </div>
                  ...
                  {msg.offer ? (
                    ...
                    <div className="flex justify-start">
                      <div className="rounded-2xl overflow-hidden" style={{
                        maxWidth: 300,
                        border: '2px solid ' + (msg.offerAccepted ? '#7DA68D' : msg.offerDeclined ? '#DDE3DE' : '#A7A5D0'),
                        background: 'white',
                      }}>
                        <div className="px-4 py-2 text-xs font-bold" style={{
                          background: msg.offerAccepted ? '#7DA68D' : msg.offerDeclined ? '#8A9192' : '#A7A5D0',
                          color: 'white',
                        }}>
                          {msg.offerAccepted ? '✅ Ponuda prihvaćena' : msg.offerDeclined ? '❌ Ponuda odbijena' : '💼 Posebna ponuda'}
                        </div>
                        <div className="p-3">
                          <p className="font-bold text-sm mb-0.5" style={{ color: '#2B3132' }}>{msg.offer.title}</p>
                          <p className="text-xs mb-1" style={{ color: '#505A5B' }}>{msg.offer.desc}</p>
                          <p className="text-base font-extrabold mb-2" style={{ color: '#A7A5D0' }}>{msg.offer.price}</p>
                          {!msg.offerAccepted && !msg.offerDeclined && (
                            <div className="flex gap-2">
                              <button onClick={() => acceptOffer(msg)}
                                className="flex-1 py-1.5 rounded-lg text-xs font-bold text-white"
                                style={{ background: '#7DA68D', border: 'none', cursor: 'pointer' }}>
                                Prihvati
                              </button>
                              <button onClick={() => declineOffer(msg.id)}
                                className="flex-1 py-1.5 rounded-lg text-xs font-bold"
                                style={{ background: '#F4F5F2', color: '#505A5B', border: '1px solid #DDE3DE', cursor: 'pointer' }}>
                                Odbij
                              </button>
                            </div>
                          )}
                    ...
                    <div className={'flex ' + (msg.from === 'user' ? 'justify-end' : 'justify-start')}>
                      <div className="max-w-xs">
                        ...
                        <p className={'text-xs mt-0.5 ' + (msg.from === 'user' ? 'text-right' : 'text-left')}
                          style={{ color: '#8A9192' }}>{msg.time}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <form onSubmit={sendMessage} className="flex items-center gap-3 px-4 py-3"
              ...
              <input type="text" placeholder="Napiši poruku..." value={input} onChange={e => setInput(e.target.value)}
